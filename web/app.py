...
def bulk_action():
    ...
    ids = [int(x) for x in request.form.getlist("followup_ids") if str(x).isdigit()]
    if not ids:
        flash("No followups selected.", "warning")
        return redirect(url_for("dashboard"))
...
from send_via_preference import render_text_email_html
...
def preview(fid):
    ...
    if email_format == "text":
        preview_mode = "html"
        plain_message = format_plain_text_message(override or description)
        branding = get_branding(user["id"]) or {}
        message = render_text_email_html(user, followup, plain_message, branding)
    else:
        preview_mode = "html"
...
#     return redirect(url_for("preview", fid=fid))
def map_send_error(e: Exception) -> str:
    msg = str(e).lower()

    # Email-related
    if "invalid recipient" in msg or "invalid address" in msg:
        return "The email address looks invalid. Please check it."

    if "authentication" in msg or "unauthorized" in msg:
        return "Your email account needs to be reconnected."

    if "quota" in msg or "limit" in msg:
        return "You’ve hit a sending limit. Try again later."

    if "timeout" in msg or "connection" in msg:
        return "Network issue. Please try again."

    if "missing" in msg and "email" in msg:
...
def template_scheduler():
    ...
    html_content = get_scheduler_template(uid) or DEFAULT_SCHEDULER_TEMPLATE

    if request.method == "POST":
        action = (request.form.get("action") or "").strip()

        if action == "reset":
            html_content = DEFAULT_SCHEDULER_TEMPLATE
        else:
            html_content = (request.form.get("html_content") or "").strip() or DEFAULT_SCHEDULER_TEMPLATE
    ...
    sample_followup = {
        ...
        "message_override": "",
        "due_date": "2026-03-25",
        "sender": user.get("name") or user.get("email") or "Support Team",
    }
...
@app.route("/branding_brand", methods=["GET", "POST"])
def branding_brand():
    user, block = require_user()
    if block:
        return block
    ...
    def save_logo_file(file_storage, user_id: int) -> str:
        if not file_storage or not file_storage.filename:
            return ""

        if not allowed_file(file_storage.filename):
            raise ValueError("Unsupported logo format. Use png, jpg, jpeg, webp, or gif.")

        ext = file_storage.filename.rsplit(".", 1)[1].lower()
        safe_name = secure_filename(file_storage.filename)
        unique_name = f"brand_{user_id}_{uuid.uuid4().hex}.{ext}"

        # adjust this folder if your app uses a different upload path
    ...
    if request.method == "POST":
        ...
        footer = (request.form.get("footer") or "").strip()

        existing = get_branding(uid) or {}

        # keep existing logo unless a new one is uploaded
        brand_logo = (existing.get("brand_logo") or existing.get("logo") or "").strip()

        logo_file = request.files.get("logo")
        if logo_file and logo_file.filename:
            try:
                brand_logo = save_logo_file(logo_file, uid)
            except ValueError as e:
                flash(str(e), "error")
        ...
        try:
            ...
            flash("Brand settings updated successfully.", "success")
            return redirect(url_for("branding_brand"))
        except Exception as e:
            app.logger.exception("Failed to save branding for user_id=%s", uid)
            flash(f"Could not save branding settings: {e}", "error")

    # ---------- GET load ----------
    branding = get_branding(uid) or {}

    # normalize keys so your template always has what it expects
    branding = {
        "brand_logo": (branding.get("brand_logo") or branding.get("logo") or "").strip(),
        "color": (branding.get("color") or branding.get("brand_color") or "#6366f1").strip() or "#6366f1",
        "company_name": (branding.get("company_name") or "").strip(),
        "support_email": (branding.get("support_email") or "").strip(),
        "footer": (branding.get("footer") or branding.get("brand_footer") or "").strip(),
    }
...
def branding():
    ...
    if request.method == "POST":
        b = get_branding(uid) or {}

        up = request.files.get("logo")
        if up and up.filename:
            safe_name = secure_filename(up.filename)
            filename = f"logo_{uuid.uuid4().hex}_{safe_name}"
            ...
            logo_url = url_for("uploaded_file", filename=filename)
        else:
            logo_url = (b.get("brand_logo") or "").strip()

        color = (request.form.get("color") or "#111827").strip()
        company_name = (request.form.get("company_name") or "").strip()
        support_email = (request.form.get("support_email") or "").strip()
        footer = (request.form.get("footer") or "").strip()
...
def normalize_logo_url(value: str) -> str:
    ...
    if v.startswith("/uploads/") or v.startswith("http://") or v.startswith("https://"):
        return v

    if "/uploads/" in v:
        return f"/uploads/{os.path.basename(v)}"
