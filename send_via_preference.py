...
def send_via_preference(user: dict, f: dict, message: str):
    ...
    if channel == "email":
        ...
        if format_type == "raw":
            ...
            raw_html = _html.unescape(message or "")
            raw_html = _extract_html_body(raw_html)

            logger.warning("[SEND_EMAIL] CALLED is_html=%s to=%s subject=%s", True, email, subject)
            logger.warning("[SEND_EMAIL] RAW HTML first 120 chars: %r", raw_html[:120])
        ...
        # -------- BRANDED HTML (default) --------
            ...
            html_body = render_followup_email_html(
                ...
                logo_url=branding.get("brand_logo") or "",
                headline="Quick follow-up",
                message_html=plain_to_html(message or ""),
                client_name=f.get("client_name") or "",
            )

            html_body = _extract_html_body(html_body)

            logger.warning("[SEND_EMAIL] CALLED is_html=%s to=%s subject=%s", True, email, subject)
            logger.warning("[SEND_EMAIL] BRANDED HTML first 120 chars: %r", html_body[:120])
